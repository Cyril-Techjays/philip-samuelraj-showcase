...
const WorldAnimation = () => {
  ...
  const countries = [
    ...
    { id: 'canada', name: 'Canada', emoji: '🇨🇦', color: '#9333ea', x: 0.2, y: 0.6 },
    { id: 'uk', name: 'UK', emoji: '🇬🇧', color: '#ec4899', x: 0.5, y: 0.2 },
    { id: 'uae', name: 'UAE', emoji: '🇦🇪', color: '#f59e0b', x: 0.75, y: 0.35 },
    { id: 'india', name: 'India', emoji: '🇮🇳', color: '#10b981', x: 0.8, y: 0.65 },
    { id: 'bangladesh', name: 'Bangladesh', emoji: '🇧🇩', color: '#ef4444', x: 0.65, y: 0.8 },
    { id: 'australia', name: 'Australia', emoji: '🇦🇺', color: '#6366f1', x: 0.35, y: 0.75 }
  ];
  ...
  const createNodes = () => {
    ...
    countries.forEach((country, index) => {
      ...
      // Add interactions
      node.addEventListener('mouseenter', () => createPulseWave(x, y, country.color));
  ...
  const createPulseWave = (x: number, y: number, color: string) => {
    ...
    wave.className = 'absolute w-[150px] h-[150px] border-2 rounded-full pointer-events-none opacity-0';
  ...
  const sendDataBurst = (fromCountry: any) => {
    const fromNode = nodesRef.current.find(n => n.country.id === fromCountry.id);
    if (!fromNode) return;
  ...
  const createDataStream = (start: any, end: any, color: string) => {
    if (!containerRef.current) return;
    
    const stream = document.createElement('div');
    stream.className = 'absolute w-[3px] h-[30px] pointer-events-none opacity-0';
  ...
  const createParticles = () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      particle.className = 'absolute w-[3px] h-[3px] rounded-full pointer-events-none opacity-60';
  ...
  const animateTitles = () => {
    ...
    const logo = containerRef.current?.querySelector('.center-logo');
